...
export default function StatisticsCharts({ t, entries, selectedMonth }: StatisticsChartsProps) {
  // Category pie chart data (current month expenses)
  ...
  const comparisonData = useMemo(() => {
    ...
    const prevExpenses = entries.filter(
      (e) => e.type === 'expense' && e.date.startsWith(prevMonth),
    );
    ...
    return [
      {
        name: t('支出', 'Expense'),
        [t('本月', 'This month')]: Math.round(sum(currentExpenses) * 100) / 100,
        [t('上月', 'Last month')]: Math.round(sum(prevExpenses) * 100) / 100,
      },
      {
        name: t('收入', 'Income'),
        [t('本月', 'This month')]: Math.round(sum(currentIncome) * 100) / 100,
        [t('上月', 'Last month')]: Math.round(sum(prevIncome) * 100) / 100,
      },
    ];
  ...
  const trendData = useMemo(() => {
    ...
    return months.map(({ key, label }) => {
      ...
      return {
        name: label,
        [t('收入', 'Income')]: Math.round(income * 100) / 100,
        [t('支出', 'Expense')]: Math.round(expense * 100) / 100,
      };
    });
  }, [entries, selectedMonth, t]);
