  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 flex flex-col items-center text-center gap-10 relative z-10">
        ...
        <button
          type="button"
          className="mt-3 text-[10px] tracking-wide text-secondary/35 transition-colors hover:text-secondary/55"
          onClick={() => {
            import('../lib/forest/forestSplashMemory').then(({ clearForestSplashMemory }) => {
              clearForestSplashMemory();
              window.dispatchEvent(new Event('forest:replay-splash'));
            });
          }}
        >
          {t('重看开屏', 'Replay intro')}
