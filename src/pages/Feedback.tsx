    ...
    >
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        >
          ...
          <h2 className="mb-2 text-xl font-bold text-on-surface">{t('发送邮件', 'Send email')}</h2>
          <p className="text-sm leading-relaxed text-secondary">
            {t(
              '会打开你的邮件客户端。邮件内容只会由你主动发送，不会先保存在本站。',
              'This opens your email client. The message is sent only when you choose to send it, and is not stored by this site first.',
            )}
          </p>
        </a>
        ...
        {FEEDBACK_URL ? (
          ...
          >
            ...
            <p className="text-sm leading-relaxed text-secondary">
              {t(
                '打开配置的反馈页面，例如 GitHub Issues。请先确认页面说明再提交。',
                'Opens the configured feedback page, such as GitHub Issues. Review that page before submitting.',
              )}
            </p>
          </a>
        ) : (
          <div className="rounded-2xl border border-dashed border-surface-variant/60 bg-surface-container-low p-6">
            ...
            <p className="text-sm leading-relaxed text-secondary">
              {t(
                '如需使用 GitHub Issues 或其他公开反馈入口，可通过 VITE_FEEDBACK_URL 配置。',
                'To use GitHub Issues or another public feedback destination, set VITE_FEEDBACK_URL.',
              )}
            </p>
          </div>
        )}
      ...
      <section className="mt-6 rounded-2xl border border-surface-variant/30 bg-white p-6 dark:bg-surface-container-high">
        ...
        <ul className="space-y-2 text-sm leading-relaxed text-secondary">
          <li>
            {t(
              '不要发送密码、验证码、证件号码、银行卡号或工具输入正文。',
              'Do not send passwords, verification codes, IDs, card numbers, or tool input text.',
            )}
          </li>
          <li>
            {t(
              '邮件会交给你的邮件服务商处理，不由本站收集。',
              'Email is handled by your email provider, not collected by this site.',
            )}
          </li>
          <li>
            {t(
              '如果某个工具需要联网，页面会按功能说明告知。',
              'If a tool requires network access, its page should say so in the feature notes.',
            )}
          </li>
        </ul>
      </section>
