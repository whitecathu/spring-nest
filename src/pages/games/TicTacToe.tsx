...
function loadScores(): { x: number; o: number; draw: number } {
  try {
    return JSON.parse(
      localStorage.getItem('spring_nest_tictactoe_scores') || '{"x":0,"o":0,"draw":0}',
    );
  } catch {
  ...
  const makeMove = useCallback((index: number, boardState: Board, player: 'X' | 'O') => {
    ...
    if (result.winner) {
      ...
      setScores((prev) => {
        const updated = {
          ...prev,
          [result.winner!.toLowerCase() as 'x' | 'o']:
            prev[result.winner!.toLowerCase() as 'x' | 'o'] + 1,
        };
        saveScores(updated);
        return updated;
      });
              ...
              return (
                <motion.button
                  ...
                  aria-label={
                    cell
                      ? t(
                          `第 ${Math.floor(i / 3) + 1} 行第 ${(i % 3) + 1} 列，${cell}`,
                          `Row ${Math.floor(i / 3) + 1}, column ${(i % 3) + 1}, ${cell}`,
                        )
                      : t(
                          `第 ${Math.floor(i / 3) + 1} 行第 ${(i % 3) + 1} 列，空位，点击落子`,
                          `Row ${Math.floor(i / 3) + 1}, column ${(i % 3) + 1}, empty, place mark`,
                        )
                  }
                  disabled={
                    !!cell || !!winner || isDraw || (mode === 'ai' && currentPlayer === 'O')
                  }
