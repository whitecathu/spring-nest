...
import { User, Settings, Shield, Bell, Key, LogOut, Check, ChevronRight, Smartphone, Globe, Moon, Sun, X, AlertTriangle } from 'lucide-react';
...
export default function Profile() {
  const { user, updateProfile, logout, language, setLanguage, t } = useUser();
  ...
  const [devices, setDevices] = useState([
    { id: '1', name: 'iPhone 14 Pro', status: 'Online · Just now', statusZh: '在线 · 刚刚', current: true, icon: 'smartphone' },
    { id: '2', name: 'MacBook Pro', status: 'Offline · 2 hours ago', statusZh: '离线 · 2小时前', current: false, icon: 'globe' }
  ]);
  ...
  const handleSaveProfile = useCallback(() => {
    updateProfile({ username: editUsername, bio: editBio, email: editEmail });
  ...
  const tabContent = useMemo(() => ({
    profile: (
      ...
      >
        <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 shadow-sm border border-surface-variant/30">
          <h3 className="text-xl font-bold text-on-surface mb-6">{t('基本信息', 'Basic Info')}</h3>

          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">{t('昵称', 'Nickname')}</label>
              <input
                type="text"
                value={editUsername}
            ...
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">{t('邮箱', 'Email')}</label>
              <input
                type="email"
                value={editEmail}
            ...
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">{t('个人简介', 'Bio')}</label>
              <textarea
                rows={4}
                value={editBio}
    ...
    notifications: (
      ...
      >
        <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 shadow-sm border border-surface-variant/30">
          <h3 className="text-xl font-bold text-on-surface mb-6">{t('消息通知', 'Notifications')}</h3>
          <p className="text-secondary mb-8">{t('选择你希望接收的通知类型，随时把握最新动态。', 'Choose the types of notifications you want to receive.')}</p>

          <div className="space-y-6">
            {[
              { key: 'email' as const, title: t('系统通知', 'System Notifications'), desc: t('安全警报、账号状态等重要信息', 'Security alerts, account status, etc.') },
              { key: 'updates' as const, title: t('更新提醒', 'Update Alerts'), desc: t('应用更新、新功能发布等', 'App updates, new features, etc.') },
              { key: 'promotions' as const, title: t('活动与促销', 'Promotions'), desc: t('春日特惠、限时活动等信息', 'Spring offers, limited-time events, etc.') },
            ].map(item => (
              <div key={item.key} className="flex items-center justify-between py-4 border-b border-surface-variant/30">
                <div>
    ...
    security: (
      ...
      >
        ...
        <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 shadow-sm border border-surface-variant/30">
          <h3 className="text-xl font-bold text-on-surface mb-6 text-red-500">{t('危险操作', 'Danger Zone')}</h3>
          <p className="text-sm text-secondary mb-6">{t('这些操作将永久影响您的帐户，请谨慎操作。', 'These actions will permanently affect your account, please proceed with caution.')}</p>
          <button onClick={() => setShowDeleteModal(true)} className="px-6 py-2.5 rounded-xl border border-red-200 dark:border-red-800 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 font-semibold transition-colors">
    ...
    settings: (
      ...
      >
        <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 shadow-sm border border-surface-variant/30">
          <h3 className="text-xl font-bold text-on-surface mb-6">{t('外观与语言', 'Appearance & Language')}</h3>
  ...
  }), [editUsername, editBio, editEmail, t, handleSaveProfile, notifications, mode, isEn, handleLanguageChange, setMode]);
  ...
  if (!user) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center py-20 px-6">
        ...
        <h1 className="font-nunito text-2xl font-bold text-on-surface mb-2">{t('未登录', 'Not Logged In')}</h1>
        <p className="text-secondary text-sm mb-6">{t('请先登录以查看个人中心', 'Please log in to view your profile')}</p>
        <button
          onClick={() => navigate('/')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 max-w-md w-full relative z-10">
            ...
            <div className="space-y-4">
              ...
              <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder={t('新密码', 'New Password')} className="w-full bg-surface-container-low dark:bg-surface-container border border-surface-variant/50 rounded-xl py-3 px-4 outline-none focus:border-primary/50" />
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('确认新密码', 'Confirm New Password')} className="w-full bg-surface-container-low dark:bg-surface-container border border-surface-variant/50 rounded-xl py-3 px-4 outline-none focus:border-primary/50" />
              {passwordError && <p className="text-red-500 text-sm">{passwordError}</p>}
            </div>
            <button onClick={handleChangePassword} className="w-full py-3 bg-primary text-on-primary rounded-xl font-bold mt-6 hover:bg-primary/90 transition-colors">{t('确认修改', 'Confirm')}</button>
          </div>
        </div>
      )}
      ...
      {showDeviceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 max-w-md w-full relative z-10">
            ...
            <div className="space-y-4">
              {devices.map(device => (
                <div key={device.id} className={`flex items-center justify-between p-4 border border-surface-variant/30 rounded-xl ${device.current ? 'bg-surface-container-lowest dark:bg-surface-container-low' : ''}`}>
                  <div className="flex items-center gap-3">
                    {device.icon === 'smartphone' ? <Smartphone className={`w-6 h-6 ${device.current ? 'text-primary' : 'text-secondary'}`} /> : <Globe className={`w-6 h-6 ${device.current ? 'text-primary' : 'text-secondary'}`} />}
                    <div>
                  ...
                  {device.current ? (
                    <span className="text-xs bg-primary-container text-primary px-2 py-1 rounded-md">{t('当前设备', 'Current')}</span>
                  ) : (
                    <button className="text-xs text-red-500 hover:underline" onClick={() => handleRemoveDevice(device.id)}>{t('移除', 'Remove')}</button>
                  )}
                </div>
              ))}
              {devices.length === 0 && <p className="text-center text-secondary text-sm">{t('无其他设备', 'No other devices')}</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-surface-container-high rounded-3xl p-8 max-w-md w-full relative z-10">
            ...
            <p className="text-secondary text-sm mb-6">{t('账号注销后，您将无法再访问任何个人数据、收藏和设置。此操作不可撤销，请三思。', 'If you delete your account, you will lose access to all your personal data, favorites, and settings. This cannot be undone.')}</p>
            <div className="flex gap-4">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-3 bg-surface-container dark:bg-surface-container-high border border-surface-variant rounded-xl font-bold text-on-surface hover:bg-surface-variant transition-colors">{t('取消', 'Cancel')}</button>
              <button onClick={handleDeleteAccount} className="flex-1 py-3 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition-colors">{t('确认注销', 'Confirm Delete')}</button>
            </div>
          </div>
        </div>
      )}
