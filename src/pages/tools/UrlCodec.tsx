  ...
  return (
    <div className="flex-grow max-w-md mx-auto w-full px-4 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-6 shadow-lg border border-surface-variant/30">
        <h2 className="text-2xl font-bold text-on-surface text-center mb-6">{t('URL 编解码', 'URL Codec')}</h2>

        {/* Mode Selector */}
        <div className="flex justify-center gap-2 mb-6">
          <button
            onClick={() => { setMode('component'); setError(''); }}
            className={`px-4 py-2 rounded-full font-semibold text-sm transition-all ${
              mode === 'component' ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-secondary hover:bg-surface-variant'
            }`}
          >
            encodeURIComponent
          </button>
          <button
            onClick={() => { setMode('uri'); setError(''); }}
            className={`px-4 py-2 rounded-full font-semibold text-sm transition-all ${
              mode === 'uri' ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-secondary hover:bg-surface-variant'
            }`}
          >
            encodeURI
          </button>
        ...
        <div className="text-xs text-secondary/60 text-center mb-4">
          {mode === 'component'
            ? t('编码所有特殊字符，适合 URL 参数值', 'Encodes all special characters, suitable for URL parameter values')
            : t('保留 URL 结构字符 (:/?#[]@!$&\'()*+,;=)', 'Preserves URL structure characters (:/?#[]@!$&\'()*+,;=)')
          }
        </div>
        ...
        {/* Output */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-secondary mb-2">{t('编码结果', 'Result')}</label>
          <div className="relative">
            <textarea
              value={output}
              readOnly
