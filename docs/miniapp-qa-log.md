...
| Gate | 范围 | 状态 | 说明 |
|------|------|------|------|
| **G0** | 脚手架 + 字体子集 + design tokens + tool-card / tool-shell / 底栏 | **UI parity wired** | brand-header + custom tabBar 已启用；需 DevTools 同屏确认 |
| **G1** | 发现 / 开发 / 效率 / 我的 + 收藏 / 记录 / 关于 + storage | **UI parity wired** | 四 Tab 已接 brand-header + custom tabBar；需 Computer Use / 人工走查 |
| **G2** | `packageStudy` 学习小筑 P0（bento 首页、题集、背/刷/考、导入、错题/收藏/统计） | code done | 首页含 bento；需并排跑通学习闭环 |
| **G3** | 其余工具工作台（29 slug，含扫码/记账/Word·PDF） | code done | tool-runtime 有全量 `case`；weather/ip/tts offline |
| **G4** | 全量 UI 对标验收 + 隐私字段 / 工程配置 / QA 文档 | **prep → launch hardening** | 主包瘦身 + 隐私空 `requiredPrivateInfos` + 提审清单；仍需 DevTools 对标 |
| **G5** | 整包回归 + P0/P1 bug 清零 | pending | 依赖 DevTools + Computer Use 全矩阵 |
...
| **word-to-pdf / pdf-to-word** | 能力边界诚实展示 | 不做虚假「完整 Office 引擎」；以本地可实现范围与明确限制文案为准 |
...
Last run: 2026-07-24 (Static QA) — `MINIAPP_VERIFY_STRICT=1 npm run verify:miniapp` **PASS**
...
- [x] `project.config.json`：春日小筑描述、touristappid、miniprogramRoot `./`
