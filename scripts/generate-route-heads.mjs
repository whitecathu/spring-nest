...
function extractCategoryRoutes() {
  ...
  const parse = (block, prefix) => [...block.matchAll(/\{\s*slug:\s*['"`]([^'"`]+)['"`],\s*category:\s*['"`]([^'"`]+)['"`],\s*label:\s*['"`]([^'"`]+)['"`],\s*labelEn:\s*['"`]([^'"`]+)['"`]/g)]
    .map((match) => ({
...
const routes = [...readFileSync(sitemapPath, 'utf8').matchAll(/<loc>https?:\/\/[^/]+([^<]+)<\/loc>/g)].map((match) => match[1]);
...
function routeMeta(route) {
  ...
  if (category) {
    ...
    return {
      ...
      description: category.type === 'tools'
  ...
  const staticMeta = {
    '/': ['免费在线实用工具与休闲小游戏合集 - Spring Nest 春日小筑', '春日小筑提供免费、轻量、无需登录、即开即用的在线实用工具与休闲小游戏。'],
    '/tools': ['实用小筑 - 免费在线工具合集', '浏览春日小筑全部免费在线工具，支持搜索、分类筛选、排序、收藏和最近使用。'],
    '/games': ['游戏天堂 - 免费休闲小游戏合集', '浏览春日小筑全部免费休闲小游戏，支持分类筛选、本地最高分和无需登录游玩。'],
    '/about': ['关于我们 - Spring Nest 春日小筑', '了解 Spring Nest 春日小筑的产品定位、隐私优先理念和工具小游戏合集。'],
    '/privacy': ['隐私政策 - Spring Nest 春日小筑', '了解春日小筑如何以本地优先方式处理收藏、最近使用、游戏分数、反馈入口和联网工具数据。'],
    '/terms': ['服务条款 - Spring Nest 春日小筑', '阅读春日小筑在线工具与休闲小游戏的使用规则、责任限制、隐私入口和反馈方式。'],
    '/feedback': ['反馈建议 - Spring Nest 春日小筑', '通过邮件或配置的反馈入口向春日小筑反馈问题和建议，不提供假提交表单。'],
    '/search': ['搜索工具和小游戏 - Spring Nest 春日小筑', '搜索春日小筑中的免费在线工具、休闲小游戏、分类和标签。'],
    '/leaderboard': ['排行榜 - Spring Nest 春日小筑', '查看春日小筑小游戏排行榜。未配置云同步时，游戏分数优先保存在浏览器本地。'],
  };
...
function replaceHead(html, route) {
  ...
  const head = [
    ...
    meta.jsonLd ? `<script type="application/ld+json">${escapeScriptJson(meta.jsonLd)}</script>` : '',
  ].filter(Boolean).join('\n    ');
  ...
  const cleanedHtml = html
    ...
    .replace(/<meta property="og:title" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta property="og:description" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta property="og:url" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta property="og:image" content="[^"]*"\s*\/?>/g, '')
    .replace(/<!-- <meta property="og:url"[\s\S]*?-->/g, '')
    .replace(/<meta name="twitter:card" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta name="twitter:title" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta name="twitter:description" content="[^"]*"\s*\/?>/g, '')
    .replace(/<meta name="twitter:image" content="[^"]*"\s*\/?>/g, '')
    .replace(/<!-- JSON-LD 结构化数据 -->\s*<script type="application\/ld\+json">[\s\S]*?<\/script>/g, '')
